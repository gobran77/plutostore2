...
export default function CustomerDashboard() {
  ...
  const isSubscriptionForCustomer = (row: any, customerId: string, customerName?: string) => {
    const ownerId = String(row?.customerId || row?.customer_id || '').trim();
    if (ownerId && ownerId === String(customerId).trim()) return true;

    const ownerName = String(row?.customerName || row?.customer_name || '').trim().toLowerCase();
    const targetName = String(customerName || '').trim().toLowerCase();
    if (targetName && ownerName && ownerName === targetName) return true;

    return false;
  };

  const loadLocalSubscriptionsForCustomer = (customerId: string, customerName?: string): Subscription[] => {
    try {
      const raw = localStorage.getItem(SUBSCRIPTIONS_STORAGE_KEY);
      if (!raw) return [];
      ...
      return parsed
        .filter((s: any) => isSubscriptionForCustomer(s, customerId, customerName))
        .map((s: any) => normalizeSubscription(s));
    } catch (e) {
  ...
  useEffect(() => {
    const initSession = async () => {
      ...
      try {
        ...
        fetchSubscriptions(normalized.id, normalized.name);
  ...
  const fetchSubscriptions = async (customerId: string, customerName?: string) => {
    try {
      const localSubs = loadLocalSubscriptionsForCustomer(customerId, customerName);
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === SUBSCRIPTIONS_STORAGE_KEY) {
        fetchSubscriptions(customer.id, customer.name);
      }
  ...
  const getStatusInfo = (subscription: Subscription) => {
    ...
    if (subscription.status === 'cancelled') {
      return { icon: XCircle, color: 'text-destructive', bgColor: 'bg-destructive/10', text: '????' };
    }
    if (daysRemaining < 0) {
      return { icon: XCircle, color: 'text-destructive', bgColor: 'bg-destructive/10', text: '?????' };
    }
    if (daysRemaining <= 3) {
      return { icon: AlertTriangle, color: 'text-warning', bgColor: 'bg-warning/10', text: '????? ??????' };
    }
    return { icon: CheckCircle, color: 'text-success', bgColor: 'bg-success/10', text: '???' };
  };

  const getRequestStatusInfo = (status: string) => {
    const normalized = String(status || '').toLowerCase();
    if (normalized === 'processing') {
      return { icon: RefreshCw, color: 'text-info', bgColor: 'bg-info/10', text: '???? ????????' };
    }
    if (normalized === 'approved') {
      return { icon: CheckCircle, color: 'text-success', bgColor: 'bg-success/10', text: '?????' };
    }
    if (normalized === 'activated') {
      return { icon: Zap, color: 'text-primary', bgColor: 'bg-primary/10', text: '?? ???????' };
    }
    ...
    return { icon: Clock, color: 'text-warning', bgColor: 'bg-warning/10', text: '????' };
  };
          ...
          <div className="p-4 bg-gradient-to-br from-primary to-primary/80 text-white">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className={`p-3 rounded-xl ${
                ...
                <p className="text-xs text-white/70 mb-1">???? ?????</p>
                  ...
                  {subscriptions.length === 0 ? (
                    ...
                    subscriptions.slice(0, 10).map((sub) => (
                      <div key={sub.id} className="p-2 rounded-md border border-border text-sm">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          ?? {format(new Date(sub.start_date), 'dd MMM yyyy', { locale: ar })} ??? {format(new Date(sub.end_date), 'dd MMM yyyy', { locale: ar })}
                        </p>
                      </div>
                    ))
                  )}
                  ...
                  {customerPayments.length === 0 ? (
                    ...
                    customerPayments.slice(0, 10).map((pay) => (
                      <div key={pay.id} className="p-2 rounded-md border border-border text-sm">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-medium">{pay.invoiceNumber || '????'}</span>
                ...
                {top.due && (
                  <div className="text-sm">
                    ...
                    {typeof top.daysLeft === 'number' && (
                      <span className="text-muted-foreground"> ({top.daysLeft >= 0 ? `????? ${top.daysLeft} ???` : `????? ${Math.abs(top.daysLeft)} ???`})</span>
                    )}
                  </div>
                )}
                    ...
                    <div key={sub.id} className="border rounded-xl p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="flex items-center gap-2 text-muted-foreground">
                          ...
                          <span>???????: {format(new Date(sub.start_date), 'dd MMM yyyy', { locale: ar })}</span>
                        </div>
                        <div className="flex items-center gap-2 text-muted-foreground">
                          <Calendar className="w-4 h-4" />
                          <span>???????: {format(new Date(sub.end_date), 'dd MMM yyyy', { locale: ar })}</span>
                        </div>
                      </div>
              ...
              <div className="border rounded-xl p-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    className="h-9"
                  >
                    {isPasskeyBusy ? '???? ????????...' : passkeyConfigured ? '????? ?????' : '????? ???? ?????'}
                  </Button>
                  <Button
                    type="button"
                    variant="outline"
