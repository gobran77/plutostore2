...
import { X, Users, User, Upload, Loader2 } from 'lucide-react';
import { ServicePricing, DBService } from '@/hooks/useServices';
...
interface EditDBServiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (id: string, service: {
    name?: string;
    description?: string;
    default_type?: string;
...
export const EditDBServiceModal = ({ isOpen, onClose, onSave, service }: EditDBServiceModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && service) {
      ...
      setDefaultType(service.default_type as 'shared' | 'private');
      setImageUrl(service.image_url);
      
      // Load pricing with defaults for missing periods
      const servicePricing = defaultPricingPeriods.map(period => {
        const existingPricing = service.pricing.find(p => p.periodDays === period.periodDays);
        if (existingPricing) {
      ...
      // Set currency from first pricing item
      if (service.pricing.length > 0) {
        setSelectedCurrency(service.pricing[0].currency);
      }
    }
  }, [isOpen, service]);
  ...
  const handleSubmit = async () => {
    if (!name.trim() || isSubmitting || !service) return;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-card rounded-2xl shadow-2xl w-full max-w-2xl mx-4 animate-scale-in border border-border max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-border sticky top-0 bg-card z-10">
          <h2 className="text-xl font-bold text-foreground">تعديل الخدمة</h2>
          <button 
            onClick={onClose} 
